...
<c:if test="${totalCnt > 0 }">
	...
	<c:forEach items="${list_tut}" var="list">
		<tr>
			...
			<c:if test="${list.user_type =='D'}"><td><span style="color:red">미승인</span></td></c:if>
			<c:if test="${list.user_type =='B'}"><td><span style="color:blue">승인</span></td></c:if>
			<td>
				<c:if test="${list.user_type =='D'}">
				<a class="btnType3 color2" href="javascript:apv_tut('${list.loginID}')"><span>승인</span></a></c:if>
