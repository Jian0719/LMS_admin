		...
		<tr>
			<td>
				<c:if test="${list.rs_check eq 'o'}">
					<input type="checkbox" id="check${list.rownum}" name="check_E" value="${list.loginId},${list.name},${list.rs_fname},${list.rs_url},${list.rs_fsize}" checked/>
				</c:if>
			...
			<td>${list.name}</td>
			<td><a href="javascript:std_Detail('${list.loginId}')">${list.loginId}</a></td>
			<td>${list.tel}</td>
			<td>${list.mail}</td>
			<td>
				<c:if test="${list.wp_state eq 'A'}">취업</c:if>
				<c:if test="${list.wp_state eq 'B'}">미취업</c:if>
			</td>
			<td>
				<c:if test="${list.rs_check eq 'o'}">
					<a href="javascript:rs_download('${list.rs_url}')">다운로드</a>
				</c:if>
			</td>			
		</tr>
</c:forEach>
